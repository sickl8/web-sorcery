...
console.log "the answer is #{answer}"`
		...
		it('creates a flattened sourcemap', () => {
			return sorcery.load('samples/1/tmp/helloworld.min.js').then((chain) => {
				...
				assert.deepEqual(map.sourcesContent, [
					sander.readFileSync('samples/1/src/helloworld.coffee').toString()
				]);
		...
		it('handles sourceMappingURLs with spaces (#6)', () => {
			return sorcery
				...
				.then((chain) => {
					...
					assert.deepEqual(map.sourcesContent, [
						sander.readFileSync('samples/4/src/file with spaces.js').toString()
					]);
	...
	describe('chain.write()', () => {
				...
				return chain.write('.tmp/write-file/helloworld.min.js').then(() => {
					return sorcery
						...
						.then((chain) => {
							...
							assert.deepEqual(map.sourcesContent, [
								sander
									.readFileSync(__dirname, 'samples/1/tmp/helloworld.coffee')
									.toString()
							]);
		...
		it('overwrites existing file', () => {
			return sander
				.copydir('samples/1/tmp')
				.to('.tmp/overwrite-file')
				.then(() => {
					return sorcery
						...
						.then((chain) => {
							return chain.write().then(() => {
								return sander
									.readFile('.tmp/overwrite-file/helloworld.min.js.map')
									.then(String)
									.then(JSON.parse)
									.then((map) => {
										const smc = new SourceMapConsumer(map);
										...
										assert.deepEqual(map.sourcesContent, [
											sander
												.readFileSync('samples/1/src/helloworld.coffee')
												.toString()
										]);
		...
		it('allows sourceMappingURL to be an absolute path', () => {
			return sorcery.load('samples/1/tmp/helloworld.min.js').then((chain) => {
				return chain
					...
					.then(() => {
						return sander
							.readFile('.tmp/helloworld.min.js')
							.then(String)
							.then((generated) => {
								const mappingURL = /sourceMappingURL=([^\s]+)/.exec(
									generated
								)[1];
								assert.equal(
									mappingURL,
		...
		it('adds a trailing newline after sourceMappingURL comment (#4)', () => {
			return sorcery.load('samples/1/tmp/helloworld.min.js').then((chain) => {
				return chain.write('.tmp/write-file/helloworld.min.js').then(() => {
					return sander
						.readFile('.tmp/write-file/helloworld.min.js')
						.then(String)
						.then((file) => {
			...
			return sorcery
				...
				.then((chain) => {
					chain.write('.tmp/with-spaces/file with spaces.js').then(() => {
						return sander
							.readFile('.tmp/with-spaces/file with spaces.js')
							.then(String)
							.then((result) => {
		...
		it('allows the base to be specified as something other than the destination file', () => {
			return sorcery.load('samples/1/tmp/helloworld.min.js').then((chain) => {
				return chain
					...
					.then(() => {
						return sander
							.readFile('.tmp/write-file/helloworld.min.js.map')
							.then(String)
							.then(JSON.parse)
							.then((map) => {
								assert.deepEqual(map.sources, [
		...
		it('writes a block comment to CSS files', () => {
			return sorcery.load('samples/5/tmp/styles.css').then((chain) => {
				return chain.write('.tmp/write-file/styles.css').then(() => {
					return sander
						.readFile('.tmp/write-file/styles.css')
						.then(String)
						.then((css) => {
		...
		it('decodes/encodes URIs', () => {
			return sorcery.load('samples/6/file with spaces.js').then((chain) => {
				return chain.write('.tmp/write-file/file with spaces.js').then(() => {
					return sander
						.readFile('.tmp/write-file/file with spaces.js')
						.then(String)
						.then((js) => {
							assert.ok(
	...
	describe('cli', () => {
		sander.readdirSync('cli').forEach((dir) => {
			...
			(/^solo-/.test(dir) ? it.only : it)(dir, (done) => {
				...
				var command = sander
					.readFileSync(dir, 'command.sh', { encoding: 'utf-8' })
					.replace(
						'sorcery',
					...
					(err, stdout, stderr) => {
						...
						function catalogue(subdir) {
							...
							return glob
								...
								.map((name) => {
									var contents = sander
										.readFileSync(subdir, name, { encoding: 'utf-8' })
										.trim();
